"""
Main Thread Executor for Charon Script Engine
...
from ..charon_logger import system_info, system_debug, system_warning, system_error, qt_message_handler
...
# Import the qt_message_handler from charon_logger instead of defining it here
...
class MainThreadExecutor(QtCore.QObject):
                ...
                if thread_override_msg and script_type.lower() == "python":
                    ...
                    if host.lower() in ['maya', 'nuke']:
                        ...
                        while time.time() - start_wait < max_wait:
                            ...
                            # Count visible windows (excluding Charon's own windows)
                            visible_windows = 0
                            for w in QtWidgets.QApplication.topLevelWidgets():
                                if w.isVisible() and hasattr(w, 'windowTitle'):
                                    title = w.windowTitle()
                                    if title and title not in ['Charon', 'Script Details', '', None]:
                                        visible_windows += 1
    ...
    def _apply_python_patches(self, script_namespace: Dict[str, Any], mirror_prints: bool, stdout_capture, stderr_capture) -> Dict[str, Any]:
        ...
        class PatchedQApplication(metaclass=PatchedQApplicationMeta):
            ...
            def exec_(self):
                ...
                while time.time() - start_time < timeout:
                    ...
                    # Check for visible non-Charon windows
                    script_windows = []
                    for w in QtWidgets.QApplication.topLevelWidgets():
                        if w.isVisible() and hasattr(w, 'windowTitle'):
                            title = w.windowTitle()
                            # Exclude known Charon windows
                            if title not in ['Charon', 'Script Details', 'Execution Details', '', None]:
        ...
        class ScriptSysModule:
            """Custom sys module that prevents script from exiting Charon"""
            def __init__(self, original_sys, mirror_prints):
                self._original = original_sys
            
            def exit(self, code=0):
                """Intercept sys.exit() to prevent closing Charon"""
                ...
                from ..qt_compat import QtWidgets
                # Close all windows except the main Charon window
                for widget in QtWidgets.QApplication.topLevelWidgets():
                    if widget.windowTitle() == "QuickQT":  # Script window
                        widget.close()
                # Don't raise SystemExit in event handlers as it may close Charon
        ...
        def patched_import(name, *args, **kwargs):
            ...
            if name == 'PySide2.QtWidgets' or (name == 'PySide2' and args and 'QtWidgets' in args[0]):
                if hasattr(module, 'QtWidgets'):
                    ...
                    if not hasattr(module.QtWidgets.QApplication, '_charon_patched'):
                        module.QtWidgets.QApplication = PatchedQApplication
                        PatchedQApplication._charon_patched = True
                    ...
                    if not hasattr(module.QApplication, '_charon_patched'):
