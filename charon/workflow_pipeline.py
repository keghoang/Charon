...
"""
        ...
        search_dirs = [comfy_dir, SCRIPT_DIR]
        env["PYTHONPATH"] = os.pathsep.join(search_dirs + ([extra_path] if extra_path else []))

        result = subprocess.run(
            [python_exe, script_path, input_path, output_path, SCRIPT_DIR, comfy_dir],
        ...
        if result.returncode != 0:
            logger.error(
                "External converter failed (code %s): %s", result.returncode, result.stderr.strip()
            )
            if strict:
                raise RuntimeError(
                    f"External converter failed: {result.stderr.strip() or result.stdout.strip()}"
                )
            return None

        if result.stdout.strip():
            logger.info("External converter output: %s", result.stdout.strip())
        if result.stderr.strip():
            logger.warning("External converter warnings: %s", result.stderr.strip())

        if not os.path.exists(output_path):
            logger.error("External converter did not produce output file: %s", output_path)
            if strict:
                raise RuntimeError("External converter did not produce output file.")
            return None

        with open(output_path, "r", encoding="utf-8") as handle:
            return json.load(handle)
    except Exception as exc:
        logger.error("External workflow conversion error: %s", exc)
        if strict:
            raise
        return None
    finally:
...
def fallback_convert(ui_workflow):
    api_workflow = {}
    ...
    for node in nodes:
        ...
        if "properties" in node:
            api_node["properties"] = node["properties"]

        widget_values = node.get("widgets_values", [])
        if node_type == "LoadImage" and widget_values:
            api_node["inputs"]["image"] = widget_values[0]
        elif node_type == "SaveImage" and widget_values:
            api_node["inputs"]["filename_prefix"] = widget_values[0]
        else:
            input_slots = [
                input_def.get("name")
                for input_def in node.get("inputs", [])
                if input_def.get("link") is None
            ]
            for index, value in enumerate(widget_values):
                if index < len(input_slots):
                    api_node["inputs"][input_slots[index]] = value

        for input_def in node.get("inputs", []):
            link_id = input_def.get("link")
            if link_id is not None:
                source = find_link_source(ui_workflow, link_id)
                if source:
                    api_node["inputs"][input_def["name"]] = [str(source[0]), source[1]]

        api_workflow[node_id] = api_node

    logger.warning("Converted workflow using basic fallback (nodes: %s)", len(api_workflow))
    return api_workflow
