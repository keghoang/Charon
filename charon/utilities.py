...
def check_version_compatibility(host=None, version=None):
    """Check if the current host version is compatible with Charon.
    ...
    if supported_versions == []:
        return False, f"{host} is not supported by Charon"
...
def setup_script_paths(script_paths=None):
    ...
    if script_paths:
        ...
        # Use first path as base, append charon_global_repo if it exists
        base_path = script_paths[0]
        potential_global_repo = os.path.join(base_path, "charon_global_repo")
...
def reload_charon_modules():
    """Reload all charon modules for fresh imports"""
    import sys
    import importlib
    
    # Get all charon modules
    charon_modules = [module for module in list(sys.modules.keys()) if module.startswith("charon")]
    
    # Remove them from sys.modules
    for module in charon_modules:
        del sys.modules[module]
    
    from .charon_logger import system_debug
    system_debug(f"Reloaded {len(charon_modules)} charon modules")
...
def get_metadata_with_fallbacks(script_path: str, current_host: str) -> dict:
    ...
    file_metadata = get_charon_config(script_path) or {}
