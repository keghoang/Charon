...
class MELExecutor(ScriptExecutor):
    ...
    def execute(self, code: str, namespace: Dict[str, Any], host: str) -> Any:
        ...
        if '__script_path__' in namespace:
            mel.eval(f'string $galt_script_path = "{namespace["__script_path__"]}";')
        ...
        try:
            ...
            # MEL scripts might set: global string $galt_return_value
            try:
                # First check if the variable exists before trying to access it
                exists = mel.eval('exists "$galt_return_value"')
                if exists:
                    mel_return = mel.eval('$galt_return_value')
                    if mel_return:
                        result = mel_return
            except:
