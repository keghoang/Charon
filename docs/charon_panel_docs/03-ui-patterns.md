...
- **Validation Column**: `ScriptTableModel` exposes a dedicated column that cycles through *Validate*, *Validating…*, *Resolve*, and *Passed*. Delegates consume `ValidationStateRole` to render state and colorize the button text.
...
- **Context Actions**: Right-clicking the Validate column offers *Revalidate*, and—when Advanced User Mode is enabled—*Show Raw Validation Payload*.
- **Per-Workflow Caching**: Validation results are restored from `%USERPROFILE%\AppData\Local\Galt\plugins\charon\Charon_repo_local\workflow\<workflow>\.charon_cache\validation\validation_resolve_status.json` during `ScriptPanel.on_scripts_loaded`, allowing offline browsing without re-hitting ComfyUI.
